  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6 md:space-y-8">
        ...
        {(studio.passiveIncomeStreams ?? []).length > 0 && (() => {
          const streams = studio.passiveIncomeStreams ?? [];
          const totalWeekly = streams.reduce((s, p) => s + p.weeklyIncome, 0);
          return (
            <div className="bg-zinc-900 border border-amber-800/30 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-bold text-amber-300">💰 Passive Income</h2>
                <span className="text-sm font-bold text-emerald-400 tabular-nums">{formatMoney(totalWeekly)}/week</span>
              </div>
              <div className="space-y-2">
                {streams.map(stream => (
                  <div key={stream.id} className="flex items-center justify-between bg-zinc-800/50 rounded-lg px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-base flex-shrink-0">{stream.type === 'merchandise' ? '🛍️' : '📺'}</span>
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-zinc-200 truncate">{stream.showTitle}</div>
                        <div className="text-xs text-zinc-500 capitalize">{stream.type === 'merchandise' ? 'Merch Deal' : `Streaming · ${stream.networkName ?? ''}`}</div>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0 ml-2">
                      <div className="text-sm font-bold text-emerald-400 tabular-nums">+{formatMoney(stream.weeklyIncome)}/wk</div>
                      <div className="text-xs text-zinc-600">until W{stream.expiresWeek} Y{stream.expiresYear}</div>
                    </div>
                  </div>
                ))}
              </div>
